...
class LBFGSLRTrainTask(ctx: TaskContext) extends TrainTask[LongWritable, Text](ctx){
  val LOG: Log = LogFactory.getLog(classOf[LBFGSLRTrainTask])
  ...
  def train(ctx: TaskContext) {
    val trainer = new LBFGSLRLearner(ctx)
